...
class PointLineView(QWidget):
    ...
    def addEdge(self, startPt, endPt, label, edgeColor, labelColor=None):
        ...
        midp = QPointF((edge.x1() + edge.x2()) / 2.0, (edge.y1() + edge.y2()) / 2.0)
    ...
    def mousePressEvent(self, e):
        ...
        self.pointclicked.emit(
            self.clicknode,
            QPointF(
                (e.x() - self.width()) / scale + 2, (self.height() - e.y()) / scale - 1
            ),
        )
    ...
    def paintEvent(self, event):
        ...
        if self.start_pt:
            ...
            pt = QPointF(
                scale * self.start_pt.x() - 0.0, scale * self.start_pt.y() - 0.0
            )
    ...
    def generateNetwork(self):
        ...
        for u in range(size):
            ...
            for i in range(OUT_DEGREE):
                ...
                pt_v = nodes[v]
                uv_len = math.sqrt(
                    (pt_v.x() - pt_u.x()) ** 2 + (pt_v.y() - pt_u.y()) ** 2
                )
    ...
    def display_paths(self, heap_path, heap_time, array_path, array_time):
        ...
        if heap_path:
            ...
            for start, end, lbl in heap_path["path"]:
                self.view.addEdge(
                    startPt=start, endPt=end, label=lbl, edgeColor=(128, 128, 255)
                )
    ...
    def checkPathInputs(self):
        if not self.graphReady:
            ...
            if not dest == "":
                ...
                self.view.setEndLoc(None)
            if (
                ((not src == self.lastPath[0]) or (not dest == self.lastPath[1]))
                and (not src == "")
                and (not dest == "")
                and (not src == dest)
            ):
    ...
    def setByClick(self, clickednode, point):
        if not self.graphReady:
            ...
            for node in self.graph.nodes:
                if (
                    math.sqrt(
                        pow((abs(node.loc.x() - point.x())), 2)
                        + pow((abs(node.loc.y() - point.y())), 2)
                    )
                    < dist
                ):
                    dist = math.sqrt(
